...
case "$os" in
	...
	(Linux)
		...
		cd /home/moonlander/nixos-config/modules/apple/brcm/source/firmware
		if [[ ${1-default} = -v ]]
		then
		...
		do
			...
			then
				if [[ ${1-default} = -v ]]
				then
					sudo cp -v $file /home/moonlander/nixos-config/modules/apple/brcm/source/firmware/brcm
				else
					sudo cp $file /home/moonlander/nixos-config/modules/apple/brcm/source/firmware/brcm
				fi
			fi
		done
